...
const { data, isFetching } = useFetch('/dashboard/v1/accounts', { initialData: [] }).json<Account[]>()
...
function getRowItems(row: Row<Account>) {
  return [
    {
      type: 'label',
      label: 'Действия'
...
const columns: TableColumn<Account>[] = [
  ...
  },
  {
    ...
    filterFn: 'equals',
    cell: ({ row }) => {
      const status = row.original.is_active ? 'Активный' : 'Неактивный';
...
<template>
  <UDashboardPanel id="accounts">
    <template #header>
      <UDashboardNavbar title="Аккаунты">
        <template #leading>
