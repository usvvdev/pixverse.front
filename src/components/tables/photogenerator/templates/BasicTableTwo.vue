...
// ====== Composable для refresh-повторений ======
...
// ====== Отслеживание видимости для lazy load ======
...
const updateTemplate = async () => {
  ...
  try {
    await withRefresh(async () => {
      ...
      const response = await fetch(`/chatgpt/api/v1/templates/${editForm.value.id}?${query}`, {
        method: 'PUT',
        body: formData,
        headers: {
...
const deleteStyle = async () => {
  ...
  try {
    await withRefresh(async () => {
      const token = localStorage.getItem('accessToken')
      const response = await fetch(`/chatgpt/api/v1/templates/${styleToDelete.value}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
